...
@Injectable()
export class ExampleRecommendationsService {
  ...
  private readonly serviceRoot = " http://ed925c21.ngrok.io";
    ...
    getExampleRecommendations(): Recommendation[] {
    ...
    /*
        devuelve las recomendaciones con 2 segundos de delay
    */

    getExampleRecommendationsDelayed(): Subject<Recommendation[]>{
        
        // Paquete de promesa
        const recommendationAvailable = new Subject<Recommendation[]>();

        setTimeout(() => {
            recommendationAvailable.next(this.getExampleRecommendations());
        }, 2000);

        return recommendationAvailable;
    }

    getRemoteRecommendations(): Observable<Recommendation[]> {
      return this.http.get(`${this.serviceRoot}/recommendations`) as Observable<Recommendation[]>;
    }

    updateRecommendation(recommendation: Recommendation){
      return this.http.put(`${this.serviceRoot}/recommendations/${recommendation.id}`,recommendation) as Observable<Recommendation>;
    }
}